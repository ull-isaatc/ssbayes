%Experimentación con la API de V-REP utilizando el EKF
...
function [Robot_1,EKF_RMS] = V_rep_funciones_V3
    ...
    semilla = 1;
        ...
        if Escena == 1 
            link = '/home/ivan/Dropbox/TFG/V-REP/Escenas/Escenario-MATLAB_V2.ttt';
        end
        if Escena == 2
            link = '/home/ivan/Dropbox/TFG/V-REP/Escenas/Escenario-MATLAB-LOW.ttt';
        end


        %Aquí es donde empieza el código principal
        disp('Nos conectaremos a la siguiente');
        IP_UNI = '10.213.13.213'
        IP_PERSONAL = '192.168.1.38'
        IP_LOCAL = '127.0.0.1'

        clientID = Abrir_conexion(vrep,IP_LOCAL);
        ...
        if conectado ==  1
            ...
            Mensaje_terminal(vrep,clientID,'Simulacion de localizacion de un robot diferencial');
            %% Preparación de la escena y definicion de parámetros
            Cargar_escena(vrep,clientID,link,1); %Cargamos la escena en la que se ejecutará la simulación
            Cargar_modelo(vrep,clientID,'/home/ivan/Dropbox/TFG/V-REP/ModeloVREP/pioneer_pen.ttm',1); %Cargamos el modelo del robot que usaremos, este es el robot real.
            
            [Estado,Handle] = Obtener_handle(vrep,clientID,'Pioneer_p3dx');
            Robot_1 = Definir_robot(vrep,clientID,Handle)
            
            Cargar_modelo(vrep,clientID,'/home/ivan/Dropbox/TFG/V-REP/ModeloVREP/pioneer_estimado.ttm',1); %Cargamos el modelo del robot que usaremos, este es el estimado.
            [Estado,Robot_2] = Obtener_handle(vrep,clientID,'Pioneer_p3dx#0');
            Definir_posicion(vrep,clientID,Robot_2,[0 0 0.1388]); %Definimos la posición inicial para el robot estimado.
            
            disp('Calculando la posición de las balizas...');
            [Pos_bal,Numero_bal] = Balizas(vrep,clientID); %Calculamos con ayuda de la función la colocación de las balizas.
            ...
            V_LINEAL = 0.7; %Velocidad lineal máxima
            V_ANGULAR = 7.2; %Velocidad de rotación máxima
            Limite_trayectoria = 500 ; %Definimos el límite de la trayectoria, es decir la cantidad máxima de puntos que cogemos.
            ...
            objetivos = Objetivos_loc(vrep,clientID,pose);
            ...
            %% Ruidos del robot
            sd_baliza = 0.00001; %Caracterizamos los ruidos que presenta el robot.
            sd_avance = 0.05;
            sd_giro = 0.00001;
            ...
            PP = [];
            %% Simulación
            Iniciar_simulacion(vrep,clientID); %Iniciamos una simulación 
            ...
            for i=1:size(objetivos,2)
                while norm(trayectoria(1:2,size(trayectoria,2))-objetivos(1:2,obj_actual)) > Epsilon && size(trayectoria,2) <= Limite_trayectoria
                    ...
                    [Estado,pose] = Obtener_pose(vrep,clientID,Robot_1.Handle_Robot);
                    Robot_1.x = pose(1,1);
                    Robot_1.y = pose(2,1);
                    ...
                    % odometría.
                    [Mov_centro ,Robot_1] = Actualizar_odom(vrep,clientID,Robot_1);
                   ...
                   % odometría, se la introduciremos a Kalman.
                    ...
                    Definir_posicion(vrep,clientID,Robot_2,[M(1,1) M(2,1) 0.1388]);
                    Definir_orientacion(vrep,clientID,Robot_2,[0 0 M(3,1)]);
                   ...
                   [w,v] = Seguir_objetivos(w,v,W,V); %Aplicamos nuestro algortimo para que el robot siga los objetivos describiendo una trayectoria curva.
                    ...
                    Mover_robot(vrep,clientID,Robot_1,v,w);
            ...
            % los objetivos.
            Parar_simulacion(vrep,clientID); %Paramos la simulación en V-REP
...
%Comenzamos a definir las funciones que utilizaremos.
    ...
    function Reanudar_simulacion(vrep,clientID) %Usaremos esta funcion en el caso de que previamente pausemos la simulacion.
    ...
    function [Estado, Handle] = Crear_dummy(vrep,clientID,size) %Creamos una esfera que nos permite hacer un test de las fisicas
        
        vrep.simxCreateDummy(clientID,size,[],vrep.simx_opmode_oneshot_wait);
    end

    function Mensaje_terminal(vrep,clientID,texto) %Funcion que nos permite mandar un mensaje via terminal.
    ...
    function [vl,vr] = Mover_robot(vrep,clientID,robot,v,w) %Funcion que nos permite que robot siga unos objetivos prestablecidos
        ...
        Enviar_signal(vrep,clientID,[vr vl]); %Enviamos la señal de movimiento al robot en V-REP.
    ...
    function [Estado]=Pausar_simulacion(vrep,clientID) %Funcion para pausar una simulacion. Esto significa que la simulacion mantendra todas las propiedades al reanudarla.
    ...
    function [Estado]= Cerrar_Escena(vrep,clientID)
